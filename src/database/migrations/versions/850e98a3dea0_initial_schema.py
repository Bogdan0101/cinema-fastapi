...
Revision ID: 850e98a3dea0
Revises:
Create Date: 2026-02-07 10:39:17.472667
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint("rating >= 1 AND rating <= 10", name="rating_1_10"),
        sa.ForeignKeyConstraint(["movie_id"], ["movies.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["user_id"], ["users.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("user_id", "movie_id", name="movie_user_unique_review"),
    )
    op.create_table(
        "user_profiles",
        sa.Column("id", sa.Integer(), autoincrement=True, nullable=False),
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("SUCCESSFUL", "CANCELED", "REFUNDED", name="paymentstatus"),
            nullable=False,
        ),
...
def downgrade() -> None:
    ...
    op.drop_table("movie_user_favorites")
